...
git pull ""   þurfum að setja eitthvað hér inn
...
Til þess að geta keyrt leikinn frá jar(bat eða eitthvað) skránni þarf viðskiptavinurinn einungis að hafa Java runtime enviroment uppsett. 
...
að geta dreift leiknum.
		...
		1. Niðurhala zip skrá frá https://github.com/rasmusMarduk/sidannaverkefniKristo  ///(þarf að breyta)
		2. Draga úr skránni, helst í heima möppuna á tölvunni. Niðurstaðan er mappa sem heitir TicTacToe //// chekka nafn a moppu
...
-------- Microsoft Windows:
	3. Viðskiptavinurinn þarf að keyra eftirfarandi skipanir í skelinni í TicTacToe möppunni:
			- mvn clean
			- mvn package(þetta mun búa til keyrslufæra .jar skrá i TicTacToe möppunni)
			- mvn install
	4. Keyrslufæra jar skráin sem inniheldur leikinn verður staðsett í eftirfarandi möppu í user home: //þarf að setja slóð hér inn samanber mylla readme.txt
	og getur verið dreifð og keyrð frá hvaða stað sem er með því að slá inn skipunina //// þarf að setja inn skipun samanber mylla readme.txt
	
	Þróunarhandbók (development manual)
	Eftirfarandi hluti þarf að gera á "hreinni" vél til þess að geta byggt(//build) og bætt við kóða á verkefninu.
	Setja upp GIT
	- www.github.com
	...
	GNU/LINUX:
	- Fara þarf á eftirfarandi slóð /// þurfum að setja inn slóð samanber mylla readme.txt
	- Næst þarf að opna skelina(terminal) og klóna verkefnið í möppu með því að nota eftirfarandi skipun
	" git clone /// hér þarf að setja inn slóð samanber mylla readme.txt"
	- Ef aðila finnst að kóði þeirra eigi skilið að tilheyra main source tree(ísl??) verður forritarinn að senda pull
	...
	fái aðgang að main repository(ísl??)
